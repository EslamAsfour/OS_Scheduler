...
public:
    ...
    float static bt_comparison(QVector <float> &v, int count, QVector <float> &gap) {
    ...
    void static sort_process_nonpre(QVector <QString> &proc_name, QVector <float> &bt, QVector <float> &arr, QVector <int> &priority, int &size, QVector <float> &gap) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (i == 0) {
                    ...
                    for (int j = i + 1; j < size; j++) {
                        if (arr[j] <= bt_comparison(bt, index, gap) && priority[j] < priority[index]) {
    ...
    void static calc_waiting_starting_gap(QVector <float> &bt, QVector <float> &arr, QVector <float> &starting, QVector <float> &waiting, QVector <float> &gap, int &size) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (i == 0) {
                    ...
                    starting[i] = bt_comparison(bt, i, gap) + arr[0];
                    if (arr[i] > starting[i]) {
